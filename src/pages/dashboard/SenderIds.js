...
import { deleteRouteFilters } from '../../redux/slices/routes';
// redux
...
import { getSenderIds } from '../../redux/slices/campaigns';
import { IdsListHead, IdsMoreMenu, IdsToolbar } from '../../sections/dashboard/campaigns/senderids';

const TABLE_HEAD = [
  { id: 'sender value', label: 'Sender Value', alignRight: false },
  { id: 'client', label: 'Client Id', alignRight: false },
  { id: 'purpose', label: 'Purpose', alignRight: false },
  // { id: 'route', label: 'Route', alignRight: true },
...
export default function RouteFilters() {
  ...
  const { senderIds } = useSelector((state) => state.campaigns);
  console.log(senderIds, 'senderIds');

  // console.log('route filters', filters);
  ...
  const handleSelectAllClick = (checked) => {
    if (checked) {
      const selected = productList.map((n) => n.filterId);
  ...
  const handleClick = (filterId) => {
    const selectedIndex = selected.indexOf(filterId);
    let newSelected = [];
    if (selectedIndex === -1) {
      newSelected = newSelected.concat(selected, filterId);
  ...
  const handleDeleteFilters = (filterId) => {
    dispatch(deleteRouteFilters(filterId));
  };
      ...
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <HeaderBreadcrumbs
          ...
          links={[
            ...
            {
              name: 'Campaigns',
              href: PATH_DASHBOARD.routes.campaigns,
            },
            { name: 'Sender Ids' },
          ]}
        />
              ...
              <Table sx={{ minWidth: 650 }} size="small">
                ...
                <TableBody>
                  {filteredProducts.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { clientId, senderValue, purpose } = row;
                    ...
                    return (
                      ...
                      >
                        <StyledTableCell padding="checkbox">
                          <Checkbox checked={isItemSelected} onClick={() => handleClick(clientId)} />
                        </StyledTableCell>
                        ...
                        <StyledTableCell>{senderValue}</StyledTableCell>
                        <StyledTableCell>{clientId}</StyledTableCell>
                        <StyledTableCell>{purpose}</StyledTableCell>
                        {/* <StyledTableCell>{filterRoute}</StyledTableCell> */}
                        {/* <StyledTableCell >{filterValue}</StyledTableCell>
                          <StyledTableCell >{filterRoute}</StyledTableCell> */}
                        <StyledTableCell align="right">
                          <IdsMoreMenu productName={clientId} onDelete={() => handleDeleteFilters(clientId)} />
                        </StyledTableCell>
                      </StyledTableRow>
                    );
                  })}
...
function applySortFilter(senderIds, comparator, query) {
  const stabilizedThis = senderIds.map((el, index) => [el, index]);
  ...
  if (query) {
    return senderIds.filter((id) => id.senderValue.toLowerCase().indexOf(query.toLowerCase()) !== -1);
  }
