...
import { getRoutes, deleteRoutes } from '../../redux/slices/routes';
...
import { MTRoute } from '../../sections/dashboard/routes';

import {MoListHead, MoToolbar, MoMoreMenu} from '../../sections/dashboard/routes/moroutes';

// const TABLE_HEAD = [
//   { id: 'owner', label: 'Owner', alignRight: false },
//   { id: 'name', label: 'Name', alignRight: false },
//   { id: 'group', label: 'Group', alignRight: false },
//   { id: 'active-binds', label: 'Active Binds', alignRight: true },
...
const TABLE_HEAD = [
  ...
  { id: 'destination', label: 'Destinantion', alignRight: false },
  // { id: 'last-activity', label: 'Last Activity', alignRight: true },
  { id: 'action', label: 'Action', alignRight: true },
];
...
export default function MtRoutes() {
  ...
  const { moroutes } = useSelector((state) => state.routes);
  // console.log('mo/mtroutes', moroutes);
              ...
              <Table sx={{ minWidth: 650 }} size="small">
                ...
                <TableBody>
                  {filteredProducts.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { routeId, routeType, order, routeSupplier, filters, rate, routeDestination } = row;
                    ...
                    return (
                      ...
                      >
                        <StyledTableCell padding="checkbox">
                          <Checkbox checked={isItemSelected} onClick={() => handleClick(routeId)} />
                        </StyledTableCell>
                        ...
                        <StyledTableCell style={{ minWidth: 160 }}>{routeType}</StyledTableCell>
                        ...
                        <StyledTableCell>{routeSupplier?.supplierName}</StyledTableCell>
                        <StyledTableCell>{filters?.filterType}</StyledTableCell>
                        <StyledTableCell>{rate}</StyledTableCell>
                        <StyledTableCell>{routeDestination}</StyledTableCell>
                        <StyledTableCell align="right">
                          <MoMoreMenu userData={routeId} onDelete={() => handleDeleteRoute(routeId)} />
                        </StyledTableCell>
                      </StyledTableRow>
                    );
                  })}
...
function applySortFilter(moroutes, comparator, query) {
  const stabilizedThis = moroutes.map((el, index) => [el, index]);
  ...
  if (query) {
    return moroutes.filter((route) => route.routeType.toLowerCase().indexOf(query.toLowerCase()) !== -1);
  }
