...
import { getSmsInbox } from '../../redux/slices/messaging';
...
import { SmsInbox } from '../../sections/dashboard/messaging';
...
export default function RouteFilters() {
  ...
  const { inbox } = useSelector((state) => state.messaging);
  ...
  useEffect(() => {
    dispatch(getSmsInbox());
  }, [dispatch]);

  useEffect(() => {
    if (inbox?.length) {
      setProductList(inbox);
    }
  }, [inbox]);
