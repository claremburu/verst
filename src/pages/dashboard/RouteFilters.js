...
import { getRouteFilters, deleteRouteFilters } from '../../redux/slices/routes';
...
import {
  FiltersToolbar, FiltersMoreMenu, FiltersListHead
} from '../../sections/dashboard/routes/filters';

const TABLE_HEAD = [
  { id: 'owner', label: 'Owner', alignRight: false },
  { id: 'filter name', label: 'Filter Name', alignRight: false },
              ...
              <Table sx={{ minWidth: 650 }} size="small">
                ...
                <TableBody>
                  {filteredProducts.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { filterId, clientId,filterName, filterValue, filterRoute, filterType, clientProfile } = row;

                    const isItemSelected = selected.indexOf(filterId) !== -1;
                    ...
                    return (
                      ...
                      >
                        <StyledTableCell padding="checkbox">
                          <Checkbox checked={isItemSelected} onClick={() => handleClick(filterId)} />
                        </StyledTableCell>
                        ...
                        <StyledTableCell >{clientProfile?.clientName}</StyledTableCell>
                        <StyledTableCell >{filterName}</StyledTableCell>
                        <StyledTableCell >{filterType}</StyledTableCell>
                        <StyledTableCell >{filterRoute}</StyledTableCell>
                        {/* <StyledTableCell >{filterValue}</StyledTableCell>
                        <StyledTableCell >{filterRoute}</StyledTableCell> */}
                        <StyledTableCell align="right">
                          <FiltersMoreMenu productName={filterId} onDelete={() => handleDeleteFilters(filterId)} />
                        </StyledTableCell>
                      </StyledTableRow>
                    );
                  })}
...
function applySortFilter(filters, comparator, query) {
  const stabilizedThis = filters.map((el, index) => [el, index]);
  ...
  if (query) {
    return filters.filter((filter) => filter.filterName.toLowerCase().indexOf(query.toLowerCase()) !== -1);
  }
