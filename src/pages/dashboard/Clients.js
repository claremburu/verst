...
import { deleteClients, getClients } from '../../redux/slices/clients';
// redux
...
import { ClientListHead, ClientMoreMenu, ClientToolbar } from '../../sections/dashboard/client';

// import { Client } from '../../sections/dashboard/client';
...
const TABLE_HEAD = [
  ...
  { id: 'last-login', label: 'Last Login', alignRight: true },
                  ...
                  {filteredProducts.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <StyledTableCell>{clientEmail}</StyledTableCell>
                        <StyledTableCell>{createdAt}</StyledTableCell>
                        <StyledTableCell>{clientAccessLevel}</StyledTableCell>
                        <StyledTableCell>
                          <Label
                            variant={isLight ? 'filled' : 'filled'}
                            color={
                              (row.clientStatus === 'Active' && 'info') ||
                              (row.clientStatus === 'Inactive' && 'error') ||
                              'error'
                            }
                          >
                            {sentenceCase(row.clientStatus)}
                          </Label>
                        </StyledTableCell>
                        <StyledTableCell>{lastLogin}</StyledTableCell>
                        <StyledTableCell align="right">
                          <ClientMoreMenu
                            userData={clientId}
                            onDelete={() => handleDeleteClient(clientId)}
                            onClientView={() => navigate(`/dashboard/view/${clientId}`)}
                          />
                        </StyledTableCell>
                      </StyledTableRow>
                    );
...
function applySortFilter(clients, comparator, query) {
  ...
  if (query) {
    return clients.filter((client) => client.clientName.toLowerCase().indexOf(query.toLowerCase()) !== -1);
  }
