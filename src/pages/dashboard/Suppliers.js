...
import { deleteSuppliers, getSuppliers } from '../../redux/slices/routes';
// redux
import { useDispatch, useSelector } from '../../redux/store';
// routes
import { PATH_DASHBOARD } from '../../routes/paths';
// utils
import { Supplier } from '../../sections/dashboard/routes';

import {
   SuppliersListHead, SuppliersMoreMenu, SuppliersToolbar
} from '../../sections/dashboard/routes/suppliers';

const TABLE_HEAD = [
  { id: 'id', label: 'Supplier ID', alignRight: false },
  { id: 'name', label: 'Supplier Name', alignRight: false },
  { id: 'preferred', label: 'Preffered', alignRight: false },
  { id: 'url', label: 'Balance URL', alignRight: false },
  { id: 'limit', label: 'Credit Limit', alignRight: false },
...
export default function Suppliers() {
  ...
  const handleDeleteSupplier = (supplierId) => {
    dispatch(deleteSuppliers(supplierId));
  }
              ...
              <Table sx={{ minWidth: 650 }} size="small">
                ...
                <TableBody>
                  {filteredProducts.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { supplierId, supplierName, supplierPreferred, balanceUrl, creditLimit, balance, bucket } =
                      row;

                    const isItemSelected = selected.indexOf(supplierId) !== -1;
                    ...
                    return (
                      ...
                      >
                        <StyledTableCell padding="checkbox">
                          <Checkbox checked={isItemSelected} onClick={() => handleClick(supplierId)} />
                        </StyledTableCell>
                        <StyledTableCell>{supplierId}</StyledTableCell>
                        <StyledTableCell>{supplierName}</StyledTableCell>
                        <StyledTableCell>{supplierPreferred}</StyledTableCell>
                        <StyledTableCell>{balanceUrl}</StyledTableCell>
                        <StyledTableCell>{creditLimit}</StyledTableCell>
                        <StyledTableCell>{balance}</StyledTableCell>
                        <StyledTableCell>{bucket}</StyledTableCell>

                        <StyledTableCell align="right">
                          <SuppliersMoreMenu userData={supplierId} onDelete={() => handleDeleteSupplier(supplierId)} />
                        </StyledTableCell>
                      </StyledTableRow>
                    );
                  })}
...
function applySortFilter(suppliers, comparator, query) {
  ...
  if (query) {
    return suppliers.filter((supplier) => supplier.supplierName.toLowerCase().indexOf(query.toLowerCase()) !== -1);
  }
