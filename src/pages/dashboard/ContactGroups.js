...
import { getContactGroups, deleteContactGroup } from '../../redux/slices/campaigns';
...
import { ContactGroups } from '../../sections/dashboard/campaigns';

import {
  ContactGroupsHead,ContactGroupsMoreMenu, ContactGroupsToolbar
} from '../../sections/dashboard/campaigns/groups';

const TABLE_HEAD = [
  // { id: 'group id', label: 'Group Id', alignRight: false },
...
export default function JasmineGroups() {
  ...
  // console.log('products', contactGroups);
              ...
              <Table sx={{ minWidth: 650 }} size="small">
                ...
                <TableBody>
                  {filteredProducts.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { groupId, name, groupName, contacts, parentId, description, clientId } = row;
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* <StyledTableCell style={{ minWidth: 160 }}>{groupId}</StyledTableCell> */}
                        ...
                        <StyledTableCell align="right">
                          <ContactGroupsMoreMenu userData={groupId} onDelete={() => handleDeleteContactGroup(groupId)} />
                        </StyledTableCell>
                      </StyledTableRow>
                    );
                  })}
...
function applySortFilter(contactGroups, comparator, query) {
  const stabilizedThis = contactGroups.map((el, index) => [el, index]);
  ...
  if (query) {
    return contactGroups.filter((group) => group.groupName.toLowerCase().indexOf(query.toLowerCase()) !== -1);
  }
