...
// import { getUsers } from '../../redux/slices/jasmine';
// redux
...
// import {
//   UsersListHead, UsersMoreMenu, UsersToolbar
// } from '../../sections/dashboard/jasmin/users';
import {
  ProviderListHead, ProviderMoreMenu, ProviderToolbar
} from '../../sections/dashboard/jasmin/provider';

import { getProviders, deleteSmppProvider } from '../../redux/slices/jasmine';
...
const TABLE_HEAD = [
  ...
  // { id: 'active-binds', label: 'Active Binds', alignRight: false },
...
export default function SnppProviders() {
  ...
  const { smppProviders } = useSelector((state) => state.jasmine);
  ...
  useEffect(() => {
    if (smppProviders?.length) {
      setProductList(smppProviders);
    }
  }, [smppProviders]);
  ...
  const handleDeleteProvider = (smppProviderId) => {
    dispatch(deleteSmppProvider(smppProviderId));
  }
              ...
              <Table sx={{ minWidth: 650 }} size="small">
                ...
                <TableBody>
                  {filteredProducts.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const {smppProviderId, accountName, session, username, status, balance, activeBinds} = row;

                    const isItemSelected = selected.indexOf(smppProviderId) !== -1;
                    ...
                    return (
                      ...
                      >
                        <TableCell padding="checkbox">
                          <Checkbox checked={isItemSelected} onClick={() => handleClick(smppProviderId)} />
                        </TableCell>
                        ...
                        <StyledTableCell align="right">
                          <ProviderMoreMenu productName={smppProviderId} onDelete={() => handleDeleteProvider(smppProviderId)} />
                        </StyledTableCell>
                      </StyledTableRow>
                    );
                  })}
